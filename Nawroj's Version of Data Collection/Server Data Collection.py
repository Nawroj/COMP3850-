...
def parse_plain_text_feed(data, source_name):
    ...
    for line in lines:
        line = line.strip()
        if line:
            # Simple IP detection (example, adapt as needed)
            ...
            if ip_match:
                results.append({'type': 'IP', 'value': line, 'source': source_name, 'timestamp': datetime.now()})
            elif url_match:
                ...
                results.append({'type': 'Unknown', 'value': line, 'source': source_name, 'timestamp': datetime.now()})
...
def load_feed_urls(filepath="/Users/ne3tii/Documents/COMP3850/Data Collection/feed_urls.json"):
...
def save_to_postgres(data, connection_string):
    """Saves parsed data to a PostgreSQL database using Supabase connection string, clearing existing data first."""
    try:
        ...
        port = parsed_url.port or 5432  # Default port if not specified

        conn = psycopg2.connect(dbname=dbname, user=user, password=password, host=host, port=port, sslmode='require')
        cur = conn.cursor()

        # Clear existing data from the indicators table
        cur.execute("TRUNCATE TABLE indicators;")  # Or "DELETE FROM indicators;" if you prefer
        conn.commit()

        for item in data:
            cur.execute(
                "INSERT INTO indicators (type, value, source, timestamp, listing_reason) VALUES (%s, %s, %s, %s, %s)",
                (item['type'], item['value'], item['source'], item['timestamp'], item.get('listing_reason')),
            )

        conn.commit()
        cur.close()
        conn.close()
        logging.info("Data saved to PostgreSQL (Supabase) successfully after clearing existing data.")
    except psycopg2.Error as e:
        logging.error(f"Error saving to PostgreSQL (Supabase): {e}")
...
# Supabase connection string (replace with your actual connection string)
supabase_connection_string = "postgresql://postgres.nvaidwhpqhldghxmfbva:A@Fp5.WGx3XF$j8@aws-0-ap-southeast-2.pooler.supabase.com:5432/postgres"

# Save data to PostgreSQL (Supabase)
save_to_postgres(all_data, supabase_connection_string)